...
Data for the "Technology Landscape" tab: high-concentration / large-volume
subcutaneous (SC) delivery platforms for monoclonal antibodies, benchmarked
against Dupixent and our internal platform.

Every entry's `source_url` was confirmed against a live search or fetch
result as of the research date below (not recalled from memory). Where a
number could not be confirmed, `concentration_mgml` is left as None and
`concentration_display` says so explicitly rather than guessing.

Last research pass: 2026-07-07
"""

from datetime import date

LAST_UPDATED = date(2026, 7, 7)

# Controlled vocabulary for phase, so the UI can badge consistently.
PHASES = [
    "Proof-of-concept",
    "Platform PoC",
    "Preclinical",
...
CATEGORY_COLOR = {
    "Liquid + excipient": "#2a78d6",       # blue
    "Enzyme co-formulation": "#eda100",    # amber
    "Suspension / particle": "#199e70",    # teal/green
    "Crystalline": "#9085e9",              # violet
    "Internal": "#e34948",                 # red/coral
}
...
ENTRIES = [
    {
        ...
        "phase": "Commercial",
        "concentration_mgml": 175,  # use the higher of the two marketed strengths for plotting
        ...
        "mechanism_long": (
            ...
            "formulation baseline that every entry below is being compared against."
        ),
        "deals": [
    ...
    },
    {
        ...
        "phase": "Internal R&D",
        "concentration_mgml": 600,
        "concentration_display": "600 mg/mL (IgG sample)",
        "mechanism_short": "[Fill in \u2014 mechanism not yet documented in this tracker]",
        "mechanism_long": (
            "Update this entry with your platform's actual mechanism, viscosity, "
            "glide-force and stability data as it becomes available. Recommended "
            "next step: generate the same syringeability and real-time/"
            "accelerated stability data package that every competitor above had "
            "to produce before any partnering conversation."
        ),
        "deals": [],
    ...
    },
    {
        ...
        "phase": "Proof-of-concept",
        "concentration_mgml": None,
        "concentration_display": "Not publicly disclosed \u2014 data gap",
        "mechanism_short": "Mechanism not publicly documented as of this research pass.",
        "mechanism_long": (
            "Confirmed to exist as a named Daewoong platform (referenced "
            "alongside their CLOPAM microneedle platform in a partnership "
            "announcement), but no English- or Korean-language source describing "
            "its mechanism, phase, or concentration data was found. Listed here "
            "as a known gap rather than guessed at \u2014 re-check periodically."
        ),
        "deals": [],
        "source_name": "FMIPA ITB partnership announcement (name-check only)",
        "source_url": "https://fmipa.itb.ac.id/2026/04/28/kembali-berkunjung-pt-daewoong-pharmaceutical-siap-matangkan-rencana-kolaborasi-riset-dengan-fmipa-itb/",
    },
    {
        "id": "regeneron_note",
